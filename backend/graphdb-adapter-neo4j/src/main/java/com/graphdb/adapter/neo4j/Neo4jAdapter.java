...
public class Neo4jAdapter implements GraphAdapter, DataHandler, SchemaHandler {
    ...
    public GraphSchema getGraphSchema(ConnectionConfig config, String graphName) throws CoreException {
        ...
        try {
            ...
            // 获取节点标签列表及其属性
            List<LabelType> vertexLabelTypes = getVertexLabelsWithProperties();
            schema.setVertexLabels(vertexLabelTypes);

            // 获取关系类型列表及其属性
            List<LabelType> edgeLabelTypes = getEdgeLabelsWithProperties();
            schema.setEdgeLabels(edgeLabelTypes);

            // 设置属性定义映射
            Map<String, List<PropertyDefinition>> propertyDefinitions = new HashMap<>();
            for (LabelType labelType : vertexLabelTypes) {
                propertyDefinitions.put("vertex:" + labelType.getName(), labelType.getProperties());
            }
            for (LabelType labelType : edgeLabelTypes) {
                propertyDefinitions.put("edge:" + labelType.getName(), labelType.getProperties());
            }
            schema.setPropertyDefinitions(propertyDefinitions);
    ...
    /**
     * 获取节点标签列表及其属性
     * 使用 CALL db.schema.nodeTypeProperties
     */
    private List<LabelType> getVertexLabelsWithProperties() {
        ...
        try {
            ...
            for (Map.Entry<String, List<PropertyDefinition>> entry : labelPropertiesMap.entrySet()) {
                ...
                labelType.setName(entry.getKey());
                labelType.setType("VERTEX");
                labelType.setDescription("Neo4j节点标签: " + entry.getKey());
    ...
    /**
     * 获取边标签列表及其属性
     * 使用 CALL db.schema.relTypeProperties
     */
    private List<LabelType> getEdgeLabelsWithProperties() {
        ...
        try {
            ...
            for (Map.Entry<String, List<PropertyDefinition>> entry : relPropertiesMap.entrySet()) {
                ...
                labelType.setDescription("Neo4j关系类型: " + entry.getKey());
                labelType.setProperties(entry.getValue());
    ...
    /**
     * 备用方法：使用 db.relationshipTypes() 获取边类型
     */
    ...
    /**
     * 将 Neo4j 数据类型映射为通用数据类型
     */
    private String mapNeo4jTypeToCommonType(String neo4jType) {
        if (neo4jType == null) {
            return "STRING";
        }
    ...
    /**
     * 将 schema 可视化返回的属性列表转换为 PropertyDefinition 列表
     */
    ...
    public GraphQueryResult executeNativeQuery(ConnectionConfig config, String graphName,
        ...
        try {
            // 验证查询语言
            if (!"Cypher".equalsIgnoreCase(queryLanguage) && !"CYPHER".equalsIgnoreCase(queryLanguage)) {
                throw new CoreException("Neo4j只支持Cypher查询语言");
            }

            // 执行原生查询
            Result neo4jResult = session.run(queryStatement);

            // 处理查询结果，提取节点和边
            List<Vertex> vertices = new ArrayList<>();
            List<Edge> edges = new ArrayList<>();

            for (Record record : neo4jResult.list()) {
                for (String key : record.keys()) {
                    Object value = record.get(key).asObject();
                    extractGraphElements(value, vertices, edges);
                }
            }
     ...
     * 从查询结果中提取图元素（节点和边）
     */
    private void extractGraphElements(Object value, List<Vertex> vertices, List<Edge> edges) {
        if (value instanceof Node) {
            // 处理节点
            Node node = (Node) value;
            Vertex vertex = convertNeo4jNodeToVertex(node);
            ...
            Edge edge = convertNeo4jRelationshipToEdge(rel);
            ...
            for (Object nestedValue : map.values()) {
                extractGraphElements(nestedValue, vertices, edges);
            }
     ...
     * 将Neo4j节点转换为Vertex对象
     */
    private Vertex convertNeo4jNodeToVertex(Node node) {
        Vertex vertex = new Vertex();
        vertex.setUid(String.valueOf(node.id()));
     ...
     * 将Neo4j关系转换为Edge对象
     */
    private Edge convertNeo4jRelationshipToEdge(Relationship rel) {
        Edge edge = new Edge();
        edge.setUid(String.valueOf(rel.id()));
        edge.setLabel(rel.type());
        edge.setSourceUid(String.valueOf(rel.startNodeId()));
        edge.setTargetUid(String.valueOf(rel.endNodeId()));
