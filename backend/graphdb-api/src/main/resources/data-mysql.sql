-- MySQL Data for GraphDB Manager
-- Generated on 2026-01-21
-- This script inserts sample data into the GraphDB Manager database for MySQL.
...
(1, 1, 1, 'MATCH (p:Person) RETURN p LIMIT 10', 234, 10, 'SUCCESS', DATE_SUB(NOW(), INTERVAL 1 DAY), 'admin'),
(2, 1, 1, 'MATCH (p:Person)-[r:KNOWS]->(f:Person) WHERE p.name = "张三" RETURN p, r, f', 456, 85, 'SUCCESS', DATE_SUB(NOW(), INTERVAL 2 HOUR), 'admin'),
(3, 2, 2, 'USE graph_space; MATCH (v:Person) RETURN v LIMIT 5', 123, 5, 'SUCCESS', DATE_SUB(NOW(), INTERVAL 5 HOUR), 'user'),
(4, 3, 3, 'g.V().hasLabel("Person").limit(10)', 345, 10, 'SUCCESS', DATE_SUB(NOW(), INTERVAL 1 HOUR), 'admin'),
(5, 1, 1, 'MATCH (p:Person) WHERE p.age > 30 RETURN count(p)', 89, 1, 'FAILED', DATE_SUB(NOW(), INTERVAL 30 MINUTE), 'user');
...
(1, 1, 1, '导入用户数据', 'users.csv', 102400, 10000, 10000, 'SUCCESS', DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 3 DAY) + INTERVAL 30 MINUTE, 'admin'),
(2, 2, 2, '导入社交关系', 'relationships.csv', 204800, 50000, 25000, 'PROCESSING', DATE_SUB(NOW(), INTERVAL 1 HOUR), NULL, 'user'),
(3, 3, 3, '导入产品数据', 'products.csv', 51200, 5000, 0, 'PENDING', NULL, NULL, 'admin');

-- 插入默认用户数据
-- 默认管理员用户：admin / admin123
-- 默认普通用户：user / user123
-- 注意：密码哈希为示例值，实际使用时应替换为真实哈希
